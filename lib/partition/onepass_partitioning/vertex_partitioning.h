...
inline void vertex_partitioning::load_edge_block(floating_block & real_block, EdgeWeight e_weight, int my_thread) {
	...
	if (real_block.leaf_block != NULL) {
		load_edge_block(*(real_block.leaf_block), e_weight, my_thread);
	}
#endif
...
inline void vertex_partitioning::propagate_load_edge(EdgeWeight e_weight, PartitionID block, int my_thread) {
	...
	if (parent_block != NULL) {
		load_edge_block(*parent_block, e_weight, my_thread);
	}
#endif
...
#ifdef MODE_STREAMMULTISECTION
	if (real_block.leaf_block != NULL) {
		clear_edgeweight_block(*(real_block.leaf_block), my_thread);
	}
#endif
...
#ifdef MODE_STREAMMULTISECTION
	if (parent_block != NULL) {
		clear_edgeweight_block(*parent_block, my_thread);
	}
#endif
...
#ifdef MODE_STREAMMULTISECTION
	if (real_block.leaf_block != NULL) {
		remove_nodeweight_block(*(real_block.leaf_block), n_weight);
	}
	real_block.parent_problem->remove_parent_nodeweight(n_weight);
#endif
}
...
#ifdef MODE_STREAMMULTISECTION
	if (real_block.child_problem != NULL) { 
		decision = real_block.child_problem->solve_node(curr_node_id, curr_node_weight, my_thread);
	}
	if (real_block.original_block != NULL) {
		decision = force_decision_block(*(real_block.original_block), curr_node_id, curr_node_weight, my_thread);
	}
#endif
	return decision;
}
...
#ifdef MODE_STREAMMULTISECTION
	if (subproblem_tree_root != NULL) {
		decision = subproblem_tree_root->solve_node(curr_node_id, curr_node_weight, my_thread);
	} else {
#endif
		if (hashing) {
