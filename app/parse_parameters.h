...
int parse_parameters(int argn, char **argv, 
	...
	// Algorithm Engineering Activities
		...
		mh_print_log,mh_sequential_mode, mh_optimize_communication_volume, mh_enable_tabu_search,
                ...
                ram_stream,  num_streams_passes,  
		parallel_nodes, suppress_file_output,
		stream_sampling, stream_sampling_type, 
		omp_schedule, omp_chunk,
		use_self_sorting_array, sampling_threashold, 

#elif defined MODE_STREAMMULTISECTION
                k, imbalance, filename_output, 
		suppress_output, suppress_file_output,
                enable_mapping, hierarchy_parameter_string, distance_parameter_string, 
		ram_stream,  
		num_streams_passes,  
		stream_rec_bisection_base, 
		parallel_nodes, omp_schedule, omp_chunk, 

		/* one_pass_algorithm, */ 
		/* fast_alg, */ 
		/* hashify_layers, */ 
		/* stream_rec_bisection, */ 
		/* stream_rec_biss_orig_alpha, stream_sampling, stream_sampling_type, */ 
		/* non_hashified_layers, percent_non_hashified_layers, */ 
		/* use_self_sorting_array, sampling_threashold, */ 
	...
	if(suppress_file_output->count > 0) {
		...
		if(!(ram_stream->count > 0)) {
                        std::cout << "--parallel_nodes requires --ram_stream." << std::endl;
                        exit(0);
		}
		...
		if(enable_mapping->count > 0) {
                        std::cout << "--stream_rec_bisection_base conflicts with --enable_mapping." << std::endl;
                        exit(0);
		}
